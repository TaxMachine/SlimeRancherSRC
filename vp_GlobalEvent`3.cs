...
public static class vp_GlobalEvent<T, U, V>
{
  ...
  public static void Register(string name, vp_GlobalCallback<T, U, V> callback)
  {
    ...
    List<vp_GlobalCallback<T, U, V>> vpGlobalCallbackList = (List<vp_GlobalCallback<T, U, V>>) m_Callbacks[name];
    if (vpGlobalCallbackList == null)
    {
      vpGlobalCallbackList = new List<vp_GlobalCallback<T, U, V>>();
  ...
  public static void Unregister(string name, vp_GlobalCallback<T, U, V> callback)
  {
    ...
    if (callback == null)
      throw new ArgumentNullException(nameof (callback));
    ((List<vp_GlobalCallback<T, U, V>>) m_Callbacks[name] ?? throw vp_GlobalEventInternal.ShowUnregisterException(name)).Remove(callback);
  }

  public static void Send(string name, T arg1, U arg2, V arg3) => Send(name, arg1, arg2, arg3, vp_GlobalEventMode.DONT_REQUIRE_LISTENER);
  ...
  public static void Send(string name, T arg1, U arg2, V arg3, vp_GlobalEventMode mode)
  {
    ...
    List<vp_GlobalCallback<T, U, V>> callback = (List<vp_GlobalCallback<T, U, V>>) m_Callbacks[name];
    if (callback != null)
    {
      foreach (vp_GlobalCallback<T, U, V> vpGlobalCallback in callback)
        vpGlobalCallback(arg1, arg2, arg3);
    }
